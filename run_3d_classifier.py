...
from keras import ops, layers, optimizers, callbacks, models
...
def transform_dataset(x):
    return x["hs_image"], ops.cast(x["class_id"], np.int32)
...
def train_model(args, shape_size, run_id):
    ...
    x = layers.Dense(args.num_classes, name="prediction", activation="softmax")(x)
    ...
    callback_list = [
        ...
        callbacks.CSVLogger(
            os.path.join(
                args.save_dir,
                "history_{}_lr_{}.csv".format(args.model, args.learning_rate),
            ),
            append=True,
        ),
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--src_dir",
        type=str,
        default="preprocessed_data/tfrecords_dataset",
    )
    ...
    parser.add_argument("--save_dir", type=str, default=f"paper_logs_3d")
    parser.add_argument("--n_runs", type=int, default=3)
    parser.add_argument("--verbose", type=int, default=0)

    args = parser.parse_args()

    save_dir = f"{args.save_dir}_{args.spatial_dim}x{args.spatial_dim}"

    # create save_dir if it does not exist
    if not os.path.exists(args.save_dir):
        os.makedirs(args.save_dir)

    shape_size = (args.src_dir, args.src_dir, 300, 1)  # todo: change if needed
    ...
    with open(
        f"{args.save_dir}/classification_report_3d_{args.model}_{args.spatial_dim}x{args.spatial_dim}.txt",
        "w",
    ) as f:
